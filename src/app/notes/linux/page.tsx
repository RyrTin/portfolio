...
function ChapterPending({ chapter }: { chapter: ChapterMeta }) {
  return (
    <div className="border border-slate-800 rounded-xl p-12 text-center opacity-50">
      <p className="text-xs font-mono text-slate-500 mb-2">{chapter.label}</p>
      <p className="text-slate-400 font-semibold text-lg">{chapter.title}</p>
...
export default function NotesLinux() {
  ...
  useEffect(() => {
    ...
    // small delay to let new chapter DOM mount
  ...
  const handleNav = useCallback((chapterId: string, sectionId?: string) => {
    ...
    if (targetId) {
      ...
      // wait for React to render new chapter, then scroll
            ...
            <div key={activeChapter}>
              ...
              <div className="border border-slate-800 rounded-xl mb-6 overflow-hidden">
                ...
                <div className="px-5 pb-6">
                  {activeChapter === 'wsl2-setup' ? <ChapterWsl2 /> : <ChapterPending chapter={activeChapterData} />}
                </div>
              </div>
            </div>
